...
std::string getVertexStr(const int _node_idx, const gtsam::Pose3& _Pose)
{
  ...
  std::string curVertexInfo{ "VERTEX_SE3:QUAT " + std::to_string(_node_idx) + " " + std::to_string(t.x()) + " " +
               ...
               std::vector<std::string>& edges_str)
{
  ...
  std::string curEdgeInfo{ "EDGE_SE3:QUAT " + std::to_string(_node_idx_pair.first) + " " +
...
{
 // std::cout<<" odometry i am here!!"<<endl;
  mBuf.lock();
...
{
  //std::cout<<" pointcloud i am here!!"<<endl;
  mBuf.lock();
...
void initNoises(void)
{
  ...
  robustNoiseVector3 << bigNoiseTolerentToXY, bigNoiseTolerentToXY,
      gpsAltitudeNoiseScore;  // means only caring altitude here. (because LOAM-like-methods tends to be asymptotically
                              // flyging)
                                ...
                                const int& root_idx)
{
  ...
  {
    int keyNear = key + i;  // see https://github.com/gisbi-kim/SC-A-LOAM/issues/7 ack. @QiMingZhenFan found the error
                            // and modified as below.
    if (keyNear < 0 || keyNear >= int(keyframeLaserClouds.size()))
      continue;

    mKF.lock();
    *nearKeyframes += *local2global(keyframeLaserClouds[keyNear], keyframePosesUpdated[root_idx]); //use the position of prior idx to turn local pointcloud to gobal pointcould of key idx 
    mKF.unlock();
  }
...
boost::optional<gtsam::Pose3> doICPVirtualRelative(int _loop_kf_idx, int _curr_kf_idx)
{
  // parse pointclouds
  ...
  {
    std::cout << "[SC loop] ICP fitness test failed (" << icp.getFitnessScore() << " > " << loopFitnessScoreThreshold
  ...
  {
    std::cout << "[SC loop] ICP fitness test passed (" << icp.getFitnessScore() << " < " << loopFitnessScoreThreshold
...
void process_pg()
{
  while (1)
  {
    ...
    // std::cout<<"odm:"<<odometryBuf.front()->header.stamp.toSec()<< "  pcd:"<<fullResBuf.front()->header.stamp.toSec()<<endl;
      // Time equal check
      ...
      const int curr_node_idx =
          keyframePoses.size() - 1;  // becuase cpp starts with 0 (actually this index could be any number, but for
                                     // simple implementation, we follow sequential indexing)
      ...
      {    // == keyframePoses.size() > 1
        ...
        mtxPosegraph.lock();
        {
          ...
          //                               curr_altitude_offseted);  // in this example, only adjusting altitude (for x and
          //                                                         // y, very big noises are set)
          //   mtxRecentPose.unlock();
...
void process_icp(void)
{
  while (1)
  {
    while (!scLoopICPBuf.empty())
    {
      ...
      {
        ROS_WARN(
            "Too many loop clousre candidates to be ICPed is waiting ... Do process_lcd less frequently (adjust "
      ...
      auto relative_pose_optional = doICPVirtualRelative(prev_node_idx, curr_node_idx);// use icp to re-check if the loop found by sc is really a loop 
...
{
  ros::init(argc, argv, "scanOptimal");
  ros::NodeHandle nh;

 // coreImportTest();

  // save directories
  nh.param<std::string>("save_directory", save_directory, "/home/yyxunn/school/smartV/temp/");  // pose assignment every k m move
